...
p.add_argument("--kick-max-steps", type=int, default=6, help="Maximum pure-lateral controller steps used to reach the measured perturbation band.")
...
def set_oracle_kick_state(env, e, angle):
    ...
    if not precontact_found:
        return False, prepare_step + 1, pre_xy, pre_z, pre_xy, pre_z, 0
    ...
    kick_steps_used = 0
    for kick_steps_used in range(1, a.kick_max_steps + 1):
        env.step(kick)
        e.sim.render()
        initial_xy, initial_z = pose_error(e)
        if initial_xy >= 0.0025:
            break
    return True, prepare_step + 1, pre_xy, pre_z, initial_xy, initial_z, kick_steps_used
...
for ep in range(a.episodes):
    ...
    policy.reset()
    precontact_found, prepare_steps_used, pre_kick_xy, pre_kick_z, initial_xy, initial_z, kick_steps_used = set_oracle_kick_state(env, e, angle)
    ...
    row = {
        ...
        "kick_steps_used": kick_steps_used,
...
summary = {
    "benchmark": "adaptive_near_rim_lateral_pulse_recovery_v3",
    ...
    "episodes": a.episodes,
    "perturbation_definition": f"same oracle controller as causal-recovery-v2 until xy<={a.pre_kick_xy_max}m and depth in ({a.pre_kick_depth_min},{a.pre_kick_depth_max}]m (max {a.prepare_max_steps} steps), then up to {a.kick_max_steps} pure-XY controller steps of magnitude {a.kick_amplitude}; angle uniform [-pi, pi]",
