...
target = np.array([0.560, 0.000, 0.115], dtype=np.float32)
position = np.array([0.820, -0.450, 0.320], dtype=np.float32)
camera = rep.create.camera(position=tuple(position.tolist()), look_at=tuple(target.tolist()), focal_length=35.0, horizontal_aperture=20.955, clipping_range=(0.01, 100.0))
...
waypoints = {
    "01_initial_lateral_error": np.array([0.012, 0.007, 0.009], dtype=np.float32),
    "02_contact_misalignment": np.array([0.006, 0.003, 0.002], dtype=np.float32),
    "03_lateral_recovery": np.array([0.0015, 0.0007, 0.000], dtype=np.float32),
    "04_aligned_insertion": np.array([0.000, 0.000, -0.003], dtype=np.float32),
}
...
# Pixel-level sanity checks used for the review record.  These reject blank
# frames and confirm that each waypoint changes the rendered scene while the
# camera and crop stay fixed.  They do not infer a policy outcome.
...
for i, (name, frame) in enumerate(zip(waypoints, rendered)):
    validation["frames"][name] = {"nonuniform": bool(float(frame.std()) > 1.0), "mean": float(frame.mean()), "std": float(frame.std())}
    if not validation["frames"][name]["nonuniform"]:
        raise RuntimeError(f"blank conceptual render: {name}")
for i in range(1, len(rendered)):
    validation["frames"][list(waypoints)[i]]["mean_abs_diff_from_initial"] = float(np.abs(rendered[i] - rendered[0]).mean())
(a.output / "render_validation.json").write_text(json.dumps(validation, indent=2) + "\n")
...
meta = {
    ...
    "scene_seed": 20260819,
    "script_or_waypoint_config": "experiment/render_simulator_conceptual_illustrations_20260819_corrected.py",
    ...
    "attachment_vector_robot_to_held_m": (base_held[:3] - base_robot[:3]).detach().cpu().tolist(),
    "lighting": {"dome_intensity": 1800.0, "key_intensity": 9000.0, "fill_intensity": 3500.0, "tonemap_exposure": 0.5},
    "states": {name: {"step": i, "simulation_step": 0, "png": f"contact_recovery_sequence/{name}.png", "waypoint_delta_xyz_m": delta.tolist()} for i, (name, delta) in enumerate(waypoints.items())},
