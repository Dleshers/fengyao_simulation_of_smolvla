...
p.add_argument("--max-initial-xy", type=float, default=0.007, help="Largest actual XY error still considered a near-rim start.")
p.add_argument("--max-initial-depth", type=float, default=0.008, help="Largest actual depth above the rim permitted at policy start.")
...
def set_perturbed_near_rim_state(e, radius, angle):
    ...
    e._held_asset.reset()
    # Freeze the current robot target: no residual reset motion may leak into evaluation.
    e.actions.zero_()
...
for ep in range(a.episodes):
    ...
    policy.reset()
    initial_xy, initial_z = set_perturbed_near_rim_state(e, radius, angle)
    ...
    for step in range(a.max_steps):
        e.sim.render()
        torque = npv(e.joint_torque[0, :7]).astype(np.float32)
        torque_trace.append(float(np.linalg.norm(torque)))
    ...
    row = {
        ...
        "initial_depth_requested_m": a.initial_depth,
        "initial_xy_error_m": initial_xy,
        "initial_depth_m": initial_z,
        "valid_non_success_initialization": bool(0.0025 <= initial_xy <= a.max_initial_xy and 0.001 < initial_z <= a.max_initial_depth),
        "success": bool(hit),
        "steps": step + 1,
...
summary = {
    "benchmark": "deterministic_hand_peg_rim_state_recovery_v1",
    "important_interpretation": "The initializer is a deterministic hand-and-peg state intervention; it is not a calibrated contact-force impulse. torque_norm is a joint-torque proxy only.",
    "torque_mode": a.torque_mode,
    "n_action_steps": cfg.n_action_steps,
    "policy": str(a.policy_path),
    "episodes": a.episodes,
    "perturbation_definition": f"deterministic root pose after hand IK: requested radius uniform [{a.perturb_min}, {a.perturb_max}]m, depth {a.initial_depth}m; accepted only if actual xy in [0.0025,{a.max_initial_xy}]m and depth in (0.001,{a.max_initial_depth}]m",
