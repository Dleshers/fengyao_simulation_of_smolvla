...
p.add_argument("--state-perturb-radius", type=float, default=0.0035, help="XY displacement applied jointly to hand and peg after a verified near-rim state.")
...
def set_oracle_kick_state(env, e, angle):
    ...
    # Controlled state perturbation: translate the grasped peg and hand together.
    # It is deliberately labelled as a state intervention, not a force impulse.
    delta = torch.zeros((e.num_envs, 3), device=e.device)
    delta[:, 0] = float(a.state_perturb_radius * np.cos(angle))
    delta[:, 1] = float(a.state_perturb_radius * np.sin(angle))
    hand_target = e.fingertip_midpoint_pos.clone() + delta
    hand_quat = e.fingertip_midpoint_quat.clone()
    env_ids = torch.arange(e.num_envs, device=e.device, dtype=torch.long)
    e.set_pos_inverse_kinematics(hand_target, hand_quat, env_ids)
    held_pose = e._held_asset.data.root_pose_w.clone()
    held_pose[:, :3] += delta
    e._held_asset.write_root_pose_to_sim(held_pose[:, :7])
    e._held_asset.write_root_velocity_to_sim(torch.zeros((e.num_envs, 6), device=e.device))
    e._held_asset.reset()
    # Freeze the current arm target so an oracle descent cannot leak into evaluation.
    e.actions.zero_()
...
for ep in range(a.episodes):
    ...
    row = {
        ...
        "state_perturb_radius_m": a.state_perturb_radius,
...
summary = {
    "benchmark": "controlled_near_rim_hand_peg_state_perturb_recovery_v4",
    ...
    "episodes": a.episodes,
    "perturbation_definition": f"same oracle controller as causal-recovery-v2 until xy<={a.pre_kick_xy_max}m and depth in ({a.pre_kick_depth_min},{a.pre_kick_depth_max}]m (max {a.prepare_max_steps} steps), then a measured hand-and-peg XY state displacement of radius {a.state_perturb_radius}m; angle uniform [-pi, pi]",
