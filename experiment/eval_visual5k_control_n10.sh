...
# Evaluate the matched visual-only 5k continuation on the same seed set used
# for visual 50k and torque-LSTM 5k.
#
# Usage:
#   Terminal A:
#     PORT=5562 bash experiment/eval_visual5k_control_n10.sh server
#
#   Terminal B:
#     PORT=5562 bash experiment/eval_visual5k_control_n10.sh client
...
EVAL_NAME="${EVAL_NAME:-targeted5_visual50k_plus_visualonly_5k_eval_n10_seed1000_20260708}"
...
case "$MODE" in
  server)
    ...
    bash _runtime/remote_handoff_gripper_lstm_work/experiment/run_eval_server.sh visual 2>&1 \
