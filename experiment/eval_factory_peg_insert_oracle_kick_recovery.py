...
def set_oracle_kick_state(env, e, angle):
    """Reproduce the collected recovery setup: oracle approach then unlabeled lateral kick."""
    for prepare_step in range(a.prepare_steps):
    ...
    return pre_xy, pre_z, initial_xy, initial_z
...
for ep in range(a.episodes):
    ...
    policy.reset()
    pre_kick_xy, pre_kick_z, initial_xy, initial_z = set_oracle_kick_state(env, e, angle)
    ...
    row = {
        ...
        "valid_non_success_initialization": bool(initial_xy >= 0.0025 and initial_z > 0.001),
        "success": bool(hit),
        "steps": step + 1,
...
summary = {
    "benchmark": "oracle_precontact_lateral_kick_recovery_v1",
    ...
    "episodes": a.episodes,
    "perturbation_definition": f"same oracle pre-contact controller as causal-recovery-v2 for {a.prepare_steps} steps, then one unlabelled XY action kick of magnitude {a.kick_amplitude}; angle uniform [-pi, pi]",
